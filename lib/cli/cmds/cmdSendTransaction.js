...
Cumulative Gas Used: ${tres.cumulativeGasUsed}`);
                ...
                case "env":
                    ...
                    env.setEnv("tx_hash", tres.transactionHash);
                    env.setEnv("tx_tindex", tres.transactionIndex);
                    env.setEnv("tx_blockNumber", tres.blockNumber);
                    env.setEnv("tx_blockHash", tres.blockHash);
                    env.setEnv("tx_gasUsed", tres.gasUsed);
                    env.setEnv("tx_cumulativeGasUsed", tres.cumulativeGasUsed);
                    env.setEnv("source", args[1]);
                    env.setEnv("dest", args[2]);
                    env.setEnv("amount", args[3]);
                    env.setEnv("dateSent", new Date());
                    env.close(envPath);
                    break;
            }
        }
