...
Transaction Index: ${txInfo.transactionIndex}`);
                ...
                case "env":
                    ...
                    env.setEnv("tx_from", txInfo.from);
                    env.setEnv("tx_to", txInfo.to);
                    env.setEnv("tx_blockHash", txInfo.blockHash);
                    env.setEnv("tx_cumulativeGasUsed", txInfo.cumulativeGasUsed);
                    env.setEnv("tx_hash", txInfo.transactionHash);
                    env.setEnv("tx_index", txInfo.transactionIndex);
                    env.close(envPath);
                    break;
            }
            return 0;
